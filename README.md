...
- 新装可配置项
  ...
  - 运行时持久化策略（可选）
    ...
    - 环境持久化（默认关闭，可按需开启）
    - 扩展持久化（可选）
      - APT 源/Key：`/etc/apt/sources.list.d`、`/etc/apt/keyrings`
      - 缓存：`/root/.npm`、`/root/go/pkg/mod`
...
4. 在“升级清单”中按编号调整目标版本、持久化策略、依赖项、扩展端口映射、扩展持久化（第9项）
5. 输入 `c` 进入确认并执行
