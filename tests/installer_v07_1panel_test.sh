...
payload=$(v07_1panel_build_compose_payload_v1 "openclaw_api_demo" "services:\n  app:\n    image: demo:latest")
assert_contains "${payload}" '"name":"openclaw_api_demo"' "payload app"
assert_contains "${payload}" '"file":"services:\\n  app:\\n    image: demo:latest"' "payload compose"

mock_tmp=$(mktemp -d)
trap 'rm -rf "${mock_tmp}"' EXIT
cat > "${mock_tmp}/curl" <<'EOT'
#!/usr/bin/env bash
if printf '%s ' "$@" | grep -q '/api/v1/health'; then
  echo '{"code":0}'
  exit 0
fi
if printf '%s ' "$@" | grep -q '/api/v1/users/profile'; then
...
if printf '%s ' "$@" | grep -q '/api/v1/containers/compose'; then
...
v07_1panel_validate_token "http://127.0.0.1:9999" "token-demo" || fail "expected token validation success"
v07_1panel_apply_compose_api_v1 "http://127.0.0.1:9999" "token-demo" "${payload}" || fail "expected apply success"
